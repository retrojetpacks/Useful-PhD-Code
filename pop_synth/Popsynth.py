...
n15_poly_c = 1.2e7
n25_poly_c = 1.7e7 #est
poly_c = n15_poly_c

#Parameters for Allona t_KH fit
Gas_pow = -1.44
Gas_c   = 6e5

plot_cols = ['#2EC4C6','#FFC82E','#CD6622','#FF1493','#440044','#6a5acd','#33cc33','#0055ff','#dd44cc','#cd6622','#A8CFFF','#62D0FF','#2EC4C6','#FFC82E','#CD6622','#FF1493','#440044','#6a5acd','#33cc33','#0055ff','#dd44cc','#cd6622','#A8CFFF','#62D0FF'
]
lss  = ['-','--',':','-.','-','-','-','-']
syms = ['o','v','s','D','p','+','2','*']
...
#fol = 'C:/Users/Colin/Documents/PhD/code/pop_synth/'
#obs_fol = 'C:/Users/Colin/Documents/PhD/code/Obs_data/'
...
def calc_analytic_disc(t,A,M_star,M_disc,args0):
    ...
    M_in = M_star + 2*np.pi*Sig*A**2 #inner disc mass
    Om_K = np.sqrt(c.G*M_in/A**3)
...
def poly_disr(M_P,R_P,R_disr):
    '''Use numerical sol for n=1.5 polytrope
    From table https://home.strw.leidenuniv.nl/~nielsen/SSEs16/Lectures-2016/lecture-7-2_2016.pdf
    '''
    Tc = poly_c * c.mu*(M_P/c.Msol) / (R_P/c.Rsol) 
    M_P = Tc * c.Msol * (R_disr/c.Rsol)  / (poly_c *c.mu ) 
    
    return M_P, R_disr, Tc


def get_poly_RP(M_P,t,t_KH):
    ''''''
    T_min = 500 #K
    T_max = 1800 #K
    Tc = (t/t_KH) * (T_max-T_min) + T_min

    R_P = (M_P/c.Msol) * (poly_c * c.mu)/ Tc  * c.Rsol
...
def solve_migration(args0):
    ...
    if RP_mode == 'Allona_fit':
        #R_P = 2*c.RJ#0.1*c.AU
        R_P = get_poly_RP(M_P,0,1)
        phase = 1
    ...
    while (t < t_max):# and (A > args0['Rin']):
        ...
        if RP_mode == 'Allona' or RP_mode == 'thermal_bath':
            ...
            t_KH = min(t_KH,Gas_c*c.sec_per_year)
            t_KH *= 10**Z**0.7
            R_P = get_poly_RP(M_P,t,t_KH)
        ...
        if R_P > R_disr:
            ...
            M_P, R_P, Tc = poly_disr(M_P,R_P,R_disr)
            if Tc < T_mid*5:
                print 'Thermal disruption!'
                M_P = 0
                R_P = 0
                t = t_max
                A = -100*c.AU
    ...
    return ts,As,M_Ps,R_Ps,RHs,Phases,alphas,M_cores,E_fbs,Us,t_20AU,typeI,Q0,M_discs,M_star
...
def run_models(params,var='Model',mode='GI_pop',timescales=False):
    ...
    if mode == 'GI_pop':
        ...
        args0['M_P0']      = 5*c.MJ  #10*c.ME #g
        args0['RP_mode']   = 'Allona_fit'#'thermal_bath'#'None'#
        args0['alpha0']    = 0.005
        args0['t_disp']    = 2e5 *c.sec_per_year #sec
        args0['t0']        = 1 #Years
        args0['Z']         = 0
        args0['DtoG']      = 0
        args0['rho_core']  = 5 #g/cm^3
    ...
    for i in range(len(Mvar)):
        for k in range(len(Mvar2)):
            ...
            ts,As,M_Ps,R_Ps,RHs,Phases,alphas,M_cores,E_fbs,Us,t_20AU,typeI,Q0,M_discs,M_star = solve_migration(args0)
...
def gas_accretion_ests():
    ...
    plt.semilogx()
    plt.xlabel('MP')
    plt.ylabel('Collapse time [years]')
...
def troup_comps(Mstar_u=1.4,Mstar_d=0.7):
    ...
    M_star = data[:,3]  #Msol
    Z_star = data[:,2]
    q_P    = M_P/ (M_star*c.Msol/c.MJ)
    
    inds = np.where((M_star>=Mstar_d) & (M_star<Mstar_u))[0]
    
    return A[inds], M_P[inds], q_P[inds]
...
def ZS09_comps(Mstar_u=1.4,Mstar_d=0.7):
    ...
    A      = data[:,6] #AU
    M_P    = data[:,5]/0.785 #MJ, Msini correction
    M_star = data[:,4]  #Msol
    q_P    = M_P/ (M_star*c.Msol/c.MJ)
...
def cumming_powerlaw(MP0,MP1,A0=0.031,A1=5):
    ...
    C calculated based on 10.5% of stars hosting a planet in mass range 0.3-10 and P 2-2000 (3AU)'''
    #Uh oh. In Cumming, M = MPxsini. Mean sini = 0.785
    mean_sini = 0.785
    MP0 *= mean_sini
    MP1 *= mean_sini
    ...
    beta  = 0.26
    beta_err = 0.1
    C = 1.04e-3 #1.46e-3 changed 0904_2019
    #A0 = min(A0,0.031)
    
    P0 = A0**(3/2)*365.25
    P1 = A1**(3/2)*365.25
    frac = C * (MP1**alpha - MP0**alpha)/alpha * (P1**beta - P0**beta)/beta

    return frac


def Cumming_2d():
    MPs = np.linspace(0.3,10,10)/0.785
    As = np.linspace(0.03,3.1,10)
    print MPs,
    print As
    P_grid = np.zeros((len(MPs),len(As)))
    
    for i in range(len(MPs)-1):
        for j in range(len(As)-1):
            P_grid[i,j] = cumming_powerlaw(MPs[i],MPs[i+1],As[j],As[j+1])
    plt.pcolormesh(As,MPs,P_grid)
    plt.semilogx()
    plt.colorbar()

    print P_grid
    print 'Total i range ', str(MPs[0]),'-',str(MPs[-1]),'MJ ',str(As[0]),'-',str(As[-1]),'AU : ', np.sum(P_grid)*100

    print cumming_powerlaw(0.3/0.785,10/0.785,0.03,3.1)*100


def load_cumming_Rdata():
    '''Cumming 2008 figure 11, MPsini>0.3MJ'''
    #fname = 'cumming_frac_period.csv'
    fname = 'cumming_num_hist.csv'
    ...
    N_sample = np.sum(num)
    Frac_tot = 0.082
    N_tot = N_sample/Frac_tot
    frac = num/N_tot
    frac_err =num_err/N_tot
    
    return a, frac, frac_err
...
def exoplanets_org(split='MP',Mstar_u=1.4,Mstar_d=0.7,plot_data=False):
    ...
    return A[inds2],M_P[inds2],q_P[inds2]
...
def histogram(params,N_combi=1000,init=False,d_hists=False,col=plot_cols[0],lw=1,ls='-',label=''):
    ...
    for i in range(N_combi):
        ...
        for j in range(params['NPs'][i]):
            ...
            ts,As,M_Ps,R_Ps,RHs,Phases,alphas,M_cores,E_fbs,Us,t_20AU,typeI,Q0,M_discs,M_starf = solve_migration(args0)
    ...
    plt.figure(20)
    plt.hist(Q0s)
    return Ais,Afs,t_20AUs,typeIs,MP0s,Mfs,Zs,Weights,Ads,Q0s,M_disc0s,M_infall0s,M_star0s,M_starfs
...
def hist_grid(disc_diag=False,d_hists=False,bar_pops=False,pos_spread=False,plot_timescales=False,survive_plot=False,
              a_MP_plot=False,N_mass_hist=False,M_hists=False,aM_scat=False,M_barplot=False,disc0_plots=False,r_hist_adv=False):
    ...
    var1 = 'M_crit'#'alpha_sg'#'alpha0'#'Z'#'M_P0'
    ...
    if r_hist_adv == True:
        fig14 = plt.figure(14)
        ax14 = fig14.add_axes([0.12,0.12,0.82,0.8])

    #==== Main histogram generation loop ====#
    dim = N_combi
    if params['M_P0'][0] == 'sweep':
        dim = N_combi * len(params['M_P0'][1:])
    big_store = np.zeros((len(params[var1]),len(params[var2]),15,dim))
    ...
    for i in range(len(params[var1])):
        for j in range(len(params[var2])):
            ...
            Ais,Afs,t_20AUs,typeIs,MP0s,MPs,Zs,weights,Ads,Q0s,M_disc0s,M_infall0s,M_star0s,M_starfs = histogram(
    ...
    if (a_MP_plot == True) and (var1 == 'M_P0'):
        ...
        #-- exo.org --#
        exo_A,exo_MP,exo_qP = exoplanets_org(split='Mstar')
    ...
    if M_hists == True:
        ...
        A_out = 5#1000 #5
        Abs = [A_in,A_out,100]
        M_bins = np.logspace(-0.5,4,30)
        q_bins = np.logspace(-4,0,30)
        for i in range(len(params[var1])):
            for j in range(len(params[var2])):
                As       = big_store[i,j,1,:]
                masses   = big_store[i,j,4,:]
                M_stars  = big_store[i,j,6,:]
                M_starfs = big_store[i,j,14,:]/c.MJ
                weights *= 100/sum(weights)
               
                for k in range(len(Abs)-1):
                    k_masses = masses[np.where((As>Abs[k]) & (As<Abs[k+1]))]
                    k_M_starfs = M_starfs[np.where((As>Abs[k]) & (As<Abs[k+1]))]
                    k_weights = weights[np.where((As>Abs[k]) & (As<Abs[k+1]))]
                    l_k =  str(params[var1][i])+', ' +str(Abs[k])+'-'+str(Abs[k+1])+' AU'
                    ax9.hist(k_masses/k_M_starfs,q_bins,label=l_k,weights=k_weights,histtype='step',color=plot_cols[i],ls=lss[k])
                    
        ax9.semilogx()
        ax9.set_xlabel(r'Mass ratio')
        ...
        for i in range(len(Obs_Ms)-1):
            Obs_per.append(cumming_powerlaw(Obs_Ms[i],Obs_Ms[i+1],A_in,A_out))
        ax9.scatter(Obs_Mmids/(c.Msol/c.MJ),np.array(Obs_per)*100,label='Cumming+08 R<5AU',color=plot_cols[9])
        
        #Tro_A,Tro_MP,Tro_qP = troup_comps()
        ...
        Rag_A,Rag_MP,Rag_q = Raghavan_comps()
        Rag_num = len(Rag_A)
        Rag_q = Rag_q[Rag_A<100]
        Rag_A = Rag_A[Rag_A<100]
        Rag_qin = Rag_q[Rag_A<A_out]
        Rag_Ain = Rag_A[Rag_A<A_out]
        Rag_qout = Rag_q[Rag_A>A_out]
        Rag_Aout = Rag_A[Rag_A>A_out]
        Rag_weightsin = np.ones(len(Rag_Ain))/Rag_num*100*0.5 #50%occurence rate
        Rag_weightsout = np.ones(len(Rag_Aout))/Rag_num*100*0.5 #50%occurence rate

        ax9.hist(Rag_qin,q_bins,label='Rag in',histtype='step',color=plot_cols[8],weights=Rag_weightsin)
        ax9.hist(Rag_qout,q_bins,label='Rag out',histtype='step',ls='--',color=plot_cols[8],weights=Rag_weightsout)
    ...
    if aM_scat == True:
        for i in range(len(params[var1])):
            for j in range(len(params[var2])):
                ...
                M_stars  = big_store[i,j,6,:]*(c.Msol/c.MJ)
                M_starfs = big_store[i,j,14,:]/c.MJ

                ax10.scatter(As,masses/M_starfs,color=plot_cols[ind],label=str(params[var1][i]),alpha=0.5,s=10)#,marker=syms[j])
                ax10.scatter(A0s,mass0s/M_stars,color=plot_cols[ind],alpha=0.1,s=5,marker=syms[1])

        ax10.semilogx()
        ax10.semilogy()
        ax10.set_xlabel('Separation [AU]')
        ax10.set_ylabel(r'Mass ratio')

        #vig17 = patches.Rectangle((20,2/1000),280,73/1000,linewidth=1,edgecolor='r',facecolor='none')
        #ax10.add_patch(vig17)

        Tro_A,Tro_MP,Tro_q = troup_comps()
        ZSo_A,ZSo_MP,ZSo_q = ZS09_comps()
        exo_A,exo_MP,exo_q = exoplanets_org(split='Mstar')
        Rag_A,Rag_MP,Rag_q = Raghavan_comps()

        ax10.scatter(exo_A,exo_q,color='#ff5555',s=3,alpha=0.5,marker=syms[-1],label=r'Exoplanet.org 0.7$M_{\odot}$<$M_*$<1.4$M_{\odot}$')
        ax10.scatter(Tro_A,Tro_q,color=plot_cols[-3],s=5,marker=syms[-2],label=r'Troup et al. 16 0.7$M_{\odot}$<$M_*$<1.4$M_{\odot}$')
        ax10.scatter(ZSo_A,ZSo_q,color=plot_cols[-4],s=5,marker=syms[-3],label=r'ZS09 0.7$M_{\odot}$<$M_*$<1.4$M_{\odot}$')
        ax10.scatter(Rag_A,Rag_q,color=plot_cols[-5],s=5,marker=syms[-4],label=r'Rag10 0.67$M_{\odot}$<$M_*$<1.22$M_{\odot}$')
        #AB Pic b. Simbad says AB Pic is a K type star, guess 800 Msol..
        ax10.errorbar(275,12/(800),yerr=2/800,color=plot_cols[3],capsize=3,label=r'AB Pic b',marker='d')
        ax10.errorbar(280,25/(800),yerr=10/800,color=plot_cols[4],capsize=3,label=r'GSC 08047-00232',marker='d')
        ax10.errorbar(63,50/(800),yerr=[[13/800],[8/800]],color=plot_cols[5],capsize=3,label=r'PZ Tel B',marker='d')
        ax10.errorbar(50,100/(800),yerr=2/800,color=plot_cols[6],capsize=3,label=r'HD 130948 BC',marker='d')

        ax10.plot([0.3,300],[21/1000,9/1000],label='Brown dwarf desert',color=plot_cols[-5])

        #NaCo-LP limit contours
        NaCo = np.genfromtxt(obs_fol+'NaCo-LP_average_detmap.txt',delimiter='  ', invalid_raise = False)
        a_NaCo = NaCo[0,1:]
        MP_NaCo = NaCo[1:,0]
        NaCo_cont = ax10.contour(a_NaCo,MP_NaCo/1000,NaCo[1:,1:]*100,levels=[0.5,5,50],colors='r',linestyles='dashed')
    ...
    if M_barplot == True:            
        ...
        for i in range(len(params[var1])):
            ...
            MPs = big_store[i,j,4,:]
            MPs[As<0] = 0
            Weights = np.ones(len(MPs))/len(MPs)*100
            M_bars,binsm = np.histogram(MPs,bins=Medges,weights=Weights)
    ...
    if disc0_plots == True:
        for i in range(len(params[var1])):
            for j in range(len(params[var2])):
                ...
                M_starfs = big_store[i,j,14,:] /c.Msol
    ...
    if r_hist_adv == True:
        #plot radius histograms for M<10, M <100, M<1000
        M_bins = [0.4,12.7,100,1000] #MJ
        ...
        for i in range(len(params[var1])):
            for j in range(len(params[var2])):
                ...
                weights *= 100/sum(weights)
                lPP = str(params[var1][i])+' PP'
                lComps = str(params[var1][i])+' Comps'
                inds_PP = np.where((masses>M_bins[0]) & (masses<M_bins[1]))
                inds_C = np.where((masses>M_bins[2]) & (masses<M_bins[3]))

                ax14.hist(As[inds_PP],A_vals,label=lPP,weights=weights[inds_PP],
                          histtype='step',color=plot_cols[i])
                ax14.hist(As[inds_C],A_vals,label=lComps,weights=weights[inds_C],
                          histtype='step',color=plot_cols[i],ls='--')

        #Obs_per = []
        #Obs_Ms = np.linspace(M_bins[0],M_bins[1],10)
        #Obs_Mmids = (Obs_Ms[1:]+Obs_Ms[:-1])/2
        #for i in range(len(A_vals[:17])):
        #    Obs_per.append(cumming_powerlaw(Obs_Ms[0],Obs_Ms[-1],A_vals[i],A_vals[i+1]))
        #ax14.scatter(A_vals[:17],np.array(Obs_per)*100,label='Cumming+08 R<5AU',color=plot_cols[9])
        
        Rag_A,Rag_MP,Rag_q = Raghavan_comps()
        Rag_weights = np.ones(len(Rag_A))/len(Rag_A)*100*0.6 #60%occurence rate
        ax14.hist(Rag_A,A_vals,label='Rag',histtype='step',ls='--',color=plot_cols[8],weights=Rag_weights)

        C08_A, frac, frac_err = load_cumming_Rdata()
        ax14.errorbar(C08_A,frac*100,yerr=frac_err*100,color=plot_cols[9],capsize=3,marker='d',label='Cumming08',ls='none')

        #Raghavan fit
        Rag_fit_As = np.logspace(-1,5,100)
        Rag_fit_Ps = Rag_fit_As**(3/2)*365 #days
        print Rag_fit_Ps
        FGK_comp_frac = 0.6 #Moe and Di Stefano 2017
        FGK_comp_per = b.lognorm_dist(Rag_fit_Ps,10**5,2.3) * FGK_comp_frac*100
        ax14.plot(Rag_fit_As,FGK_comp_per)
        
        ax14.legend(frameon=False)
        ax14.set_xlabel('Separation [AU]')
        ax14.set_ylabel('Percentage of FGK systems')
        ax14.semilogx()
    return 
...
if __name__ == '__main__':
    ...
    #run_models(params=['A','M_P','R_P','M_disc'],var='M_P0',timescales=False,mode='GI_gasacc')#var='M_P0')#'SG_corr') #'M_P',#'alpha'
    ...
    hist_grid(M_hists=True,aM_scat=True,M_barplot=True,r_hist_adv=True,disc0_plots=True) 
    #hist_grid(r_hist_adv=True) 
