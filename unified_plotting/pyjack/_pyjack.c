...
static char pyjack_docstring[] =
    "Arguments: sph_x, sph_y, sph_h, sph_A, sph_vx, sph_vy, nbins, lower, upper, kernel_n, M_sph";
...
static PyObject *smoother_pyjack(PyObject *self, PyObject *args)
{
    ...
    PyObject *sph_x_obj, *sph_y_obj, *sph_h_obj, *sph_A_obj, *sph_vx_obj, *sph_vy_obj;
    printf("Function Start");
    
    /* Parse the input tuple */
    if (!PyArg_ParseTuple(args, "OOOOOOidddd", &sph_x_obj, &sph_y_obj, &sph_h_obj, &sph_A_obj,
			  &sph_vx_obj, &sph_vy_obj,
    ...
    PyObject *sph_vx_array = PyArray_FROM_OTF(sph_vx_obj, NPY_DOUBLE, NPY_IN_ARRAY);
    PyObject *sph_vy_array = PyArray_FROM_OTF(sph_vy_obj, NPY_DOUBLE, NPY_IN_ARRAY);
    
    /* If that didn't work, throw an exception. */
    if (sph_x_array == NULL || sph_y_array == NULL || sph_h_array == NULL || sph_A_array == NULL || sph_vx_array == NULL || sph_vy_array == NULL) {
        Py_XDECREF(sph_x_array);
        Py_XDECREF(sph_y_array);
	Py_XDECREF(sph_h_array);
	Py_XDECREF(sph_A_array);
	Py_XDECREF(sph_vx_array);
        Py_XDECREF(sph_vy_array);
        return NULL;
    }
    ...
    double *sph_vx   = (double*)PyArray_DATA(sph_vx_array);
    double *sph_vy   = (double*)PyArray_DATA(sph_vy_array);
    double *rendered = (double*)malloc(4*ntot*sizeof(double));

    /* Do the computation */
    double width = (upper-lower)/nbins;
    double * bins = malloc(nbins*sizeof(double));
    for (i=0; i<nbins; i++) bins[i] = lower+(i+0.5)*width;
    for (i=0; i<4*nbins; i++) rendered[i] = 0;
	...
	double vy    = sph_vy[i];
        ...
        for (j=mx_lower; j<mx_upper; j++) {
		  ...
		  rendered[2*ntot+j+nbins*k] += temp * vx;
		  rendered[3*ntot+j+nbins*k] += temp * vy;
		}
    ...
    npy_intp dims[1] = {4*ntot};
    PyObject *rendered_return  = PyArray_SimpleNew(1, dims, NPY_DOUBLE);
    memcpy(PyArray_DATA(rendered_return), rendered, sizeof(double)*4*nbins*nbins);
