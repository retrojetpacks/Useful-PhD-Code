...
static char pyjack_docstring[] =
    "Arguments: sph_x, sph_y, sph_h, nbins, lower, upper, kernel_n, M_sph";
...
static PyObject *smoother_pyjack(PyObject *self, PyObject *args)
{
    ...
    PyObject *sph_x_obj, *sph_y_obj, *sph_h_obj;
    printf("Function Start");
    
    /* Parse the input tuple */
    if (!PyArg_ParseTuple(args, "OOOidddd", &sph_x_obj, &sph_y_obj, &sph_h_obj, &nbins,
			  &lower, &upper, &kernel_n, &M_sph))
        return NULL;
    ...
    if (sph_x_array == NULL || sph_y_array == NULL || sph_h_array == NULL) {
	...
	double one_h2 = 1 / h2;
        ...
        int mx_lower = (int)floor((x_pos - lower - sph_h[i])/width - 1.5);
        int mx_upper = (int)floor((x_pos - lower + sph_h[i])/width + 1.5);
        if (mx_lower<0) mx_lower=0;
        if (mx_upper>nbins-1) mx_upper=nbins-1;

        int my_lower = (int)floor((y_pos - lower - sph_h[i])/width - 1.5);
        int my_upper = (int)floor((y_pos - lower + sph_h[i])/width + 1.5);
        ...
        for (j=mx_lower; j<mx_upper; j++) {
	        ...
	        double dist2 = dist_x + (y_pos-bins[k])*(y_pos-bins[k]);
                if (dist2<h2) {
		  //rendered[j+nbins*k] += pow(1 - (dist2/h2), 4) / h2;
		  double kern =  (1 - dist2 * one_h2)*(1- dist2*one_h2);
		  rendered[j+nbins*k] += kern*kern * one_h2;
		}
    ...
    npy_intp dims[1] = {nbins*nbins};
    PyObject *rendered_return  = PyArray_SimpleNew(1, dims, NPY_DOUBLE);
    memcpy(PyArray_DATA(rendered_return), rendered, sizeof(double)*nbins*nbins);
